...
impl SqliteUtils {
    ...
    pub fn row_to_record(row: &sqlx::sqlite::SqliteRow, schema: &'static CollectionSchema) -> Result<StorageRecord, DatabaseError> {
        let mut record = StorageRecord::new();
        let sequence: i64 = row.try_get(COLUMN_SEQUENCE).map_err(|e| DatabaseError::Other(e.to_string()))?;
        let uid: String = row.try_get(COLUMN_ID).map_err(|e| DatabaseError::Other(e.to_string()))?;
        let created_at_str: String = row.try_get(COLUMN_CREATED_AT).map_err(|e| DatabaseError::Other(e.to_string()))?;
        let updated_at_str: String = row.try_get(COLUMN_UPDATED_AT).map_err(|e| DatabaseError::Other(e.to_string()))?;
        ...
        let permissions_json: String = row.try_get(COLUMN_PERMISSIONS).map_err(|e| DatabaseError::Other(e.to_string()))?;
        let permissions: Vec<String> = serde_json::from_str(&permissions_json).map_err(|e| DatabaseError::Other(e.to_string()))?;
        ...
        for attr in schema.persisted_attributes() {
            ...
            let value = if attr.array {
                match row.try_get::<Option<String>, _>(key) {
                    Ok(Some(json)) => {
                        let val: serde_json::Value = serde_json::from_str(&json).map_err(|e| DatabaseError::Other(e.to_string()))?;
                ...
                match attr.kind {
                    AttributeKind::String | AttributeKind::Relationship | AttributeKind::Virtual | AttributeKind::Json => {
                        row.try_get::<Option<String>, _>(key).map(|v| v.map(StorageValue::String).unwrap_or(StorageValue::Null)).unwrap_or(StorageValue::Null)
                    }
                    AttributeKind::Integer | AttributeKind::Boolean => {
                        let v = row.try_get::<Option<i64>, _>(key).unwrap_or(None);
                        match v {
                            Some(val) => if attr.kind == AttributeKind::Boolean { StorageValue::Bool(val != 0) } else { StorageValue::Int(val) },
                            None => StorageValue::Null,
                        }
                    }
                    AttributeKind::Float => row.try_get::<Option<f64>, _>(key).map(|v| v.map(StorageValue::Float).unwrap_or(StorageValue::Null)).unwrap_or(StorageValue::Null),
                    AttributeKind::Timestamp => {
                        let s = row.try_get::<Option<String>, _>(key).unwrap_or(None);
                        match s {
                            Some(val) => {
                                let dt = OffsetDateTime::parse(&val, &time::format_description::well_known::Rfc3339).map_err(|e| DatabaseError::Other(e.to_string()))?;
    ...
    pub fn json_to_storage_value(val: serde_json::Value, kind: AttributeKind, array: bool) -> Result<StorageValue, DatabaseError> {
        if array {
            let serde_json::Value::Array(elems) = val else { return Err(DatabaseError::Other("expected array".into())); };
            match kind {
                AttributeKind::String | AttributeKind::Relationship | AttributeKind::Virtual | AttributeKind::Json => {
                ...
                AttributeKind::Timestamp => {
                    ...
                    for e in elems {
                        let s = e.as_str().ok_or_else(|| DatabaseError::Other("expected string".into()))?;
                        dates.push(OffsetDateTime::parse(s, &time::format_description::well_known::Rfc3339).map_err(|e| DatabaseError::Other(e.to_string()))?);
                    }
